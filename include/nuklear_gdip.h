...
float nk_gdipfont_get_text_width(nk_handle handle, float height, const char *text, int len);
...
nk_gdipfont_create_from_memory(const void *membuf, int membufSize, int size)
{
    if (!nk_gdip_getCurFontCollection())
        if (GdipNewPrivateFontCollection(&gdip.fontCollection[gdip.curFontCollection]))
            return NULL;
    if (GdipPrivateAddMemoryFont(nk_gdip_getCurFontCollection(), membuf, membufSize))
        return NULL;
...
nk_gdipfont_create_from_file(const WCHAR *filename, int size)
{
    if (!nk_gdip_getCurFontCollection())
        if (GdipNewPrivateFontCollection(&gdip.fontCollection[gdip.curFontCollection]))
            return NULL;
    if (GdipPrivateAddFontFile(nk_gdip_getCurFontCollection(), filename))
        return NULL;
    return nk_gdipfont_create_from_collection(size);
}
...
{
    int i;
    GdiplusStartupInput startup = GdiplusStartupInput();
...
nk_gdip_prerender_gui(enum nk_anti_aliasing AA)
{
    ...
    GdipSetTextRenderingHint(gdip.memory, AA != NK_ANTI_ALIASING_OFF ? TextRenderingHintClearTypeGridFit : TextRenderingHintSingleBitPerPixelGridFit);
    GdipSetSmoothingMode(gdip.memory, AA != NK_ANTI_ALIASING_OFF ? SmoothingModeHighQuality : SmoothingModeNone);
